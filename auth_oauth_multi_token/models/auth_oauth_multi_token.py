...
class AuthOauthMultiToken(models.Model):
    ...
    logout_on = fields.Datetime('Logged Out On')

    user_id = fields.Many2one(comodel_name='res.users', string='User', required=True, readonly=True)

    _sql_constraints = [
        ('uniq_users_device_id_user_id', 'unique(device_id, user_id)', 'Device ID must be unique per user'),
    ]
