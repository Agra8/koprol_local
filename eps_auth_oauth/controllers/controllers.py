...
class EpsAuthOauth():
    ...
    def check_token(func):
        ...
        def wrap(self, *args, **kwargs):
            ...
            try:
                user = request.env['res.user'].sudo().search(
                ...
                validation = request.env['res.users'].sudo().verify_token(user.user_id.oauth_provider_id.id, access_token, user.user_id.oauth_uid)
                if not validation:
