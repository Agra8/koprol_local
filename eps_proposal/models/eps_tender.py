...
class Tender(models.Model):
    ...
    def action_request_approval(self):
        ...
        for line in self.tender_participant_ids:
            if not line.sudo().employee_id.job_id.group_id:
                raise ValidationError('Perhatian ! User Group (%s) Karyawan (%s) belum diisi di Master Job.' % (line.sudo().employee_id.job_id.name,line.employee_id.name))
    ...
    def get_full_url(self):
        ...
        url_params = {
            ...
            'action': self.env.ref('eps_proposal.eps_tender_action').id,
        }
